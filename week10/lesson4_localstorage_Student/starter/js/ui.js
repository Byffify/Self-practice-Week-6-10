...
function saveQuotes() {
  if (typeof getAllQuotes !== 'function') return quotes = []
  const result = getAllQuotes()
  if (!Array.isArray(result)) return quotes = []
  const quote = result
  const newQuote = JSON.stringify(quote)
  localStorage.setItem('saveQuote', newQuote)
}

function loadQuotes() {
  const saveQuote = localStorage.getItem('saveQuote')
  if (!saveQuote) return
  const newQuote = JSON.parse(saveQuote)
